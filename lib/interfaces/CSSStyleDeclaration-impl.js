...
} = require("../serializers/declarations");

const {
  serializeACSSComponentValue,
} = require("../serializers/values");
...
class CSSStyleDeclarationImpl {
  constructor(args, { ownerNode, parentCSSRule, readOnly, rule = createRule() }) {
    this._ownerNode = ownerNode;
    this._parentCSSRule = parentCSSRule;
    this._readOnly = readOnly;
    this._rule = rule;
    this._declarations = this._rule.block.children;
  }

  get [idlUtils.supportedPropertyNames]() {
    return this._declarations.toArray().keys();
  }
  ...
  get cssText() {
    // FIXME
    return simpleSerializeCSSDeclarationBlock(this._rule.block);
  }
  set cssText(value) {
    // If the readonly flag is set, throw a NoModificationAllowedError exception and terminate these steps.
    if (this._readOnly) {
      throw new DOMException("getComputedStyle's CSSStyleDeclaration is not mutable", "NoModificationAllowedError");
    }

    // Empty the declarations.
    this._declarations.clear();

    // Parse the given value and, if the return value is not the empty list,
    // insert the items in the list into the declarations, in specified order.
    const parsedDeclarationList = parse(value, { context: "declarationList" });

    if (!parsedDeclarationList.children.isEmpty()) {
      //! Specified order means to expand shorthand properties in canonical order
      parsedDeclarationList.children.each(declaration => this._declarations.appendList(expandShorthand(declaration)));
    }
  }

  get length() {
    // The length attribute must return the number
    // of CSS declarations in the declarations.
    return this._declarations.getSize();
  }

  item(index) {
    // The item(index) method must return the property name
    // of the CSS declaration at position index.
    const list = this._declarations.toArray()[index];
  ...
  getPropertyValue(property) {
    ...
    if (!isCustomProperty(property)) {
      ...
      if (isShorthandProperty(property)) {
        ...
        // For each longhand property longhand that property maps to,
        // in canonical order, follow these substeps:
        // TODO

        // If longhand is a case-sensitive match for a property name of
        // a CSS declaration in the declarations, let declaration be
        // that CSS declaration, or null otherwise.
        ...
        if (propertiesHaveSameImportance(list)) {
          return serializeACSSComponentValue(list);
        }
    ...
    // If property is a case-sensitive match for a property name of
    // a CSS declaration in the declarations, return the result of
    // invoking serialize a CSS value of that declaration and terminate these steps.
    let matchingDeclarationValue;

    walkDeclarations(this._rule, node => {
      if (property === node.property) {
        matchingDeclarationValue = serializeACSSComponentValue(node.value);
      }
    });

    if (matchingDeclarationValue) {
      return matchingDeclarationValue;
    }
  ...
  setProperty(property, value, priority) {
    // If the readonly flag is set, throw a NoModificationAllowedError exception and terminate these steps.
    if (this._readOnly) {
      throw new DOMException("The object can not be modified", "NoModificationAllowedError");
    }
    ...
    if (priority && priority !== "" && !asciiCaseInsensitiveMatch(priority, "important")) {
      return;
    }
    ...
    // this._declarations.appendList(withExpandedShorthand);
    ...
    this._declarations.insertList(withExpandedShorthands);
  }
