...
class CSSStyleSheetImpl extends StyleSheetImpl {
  constructor(args, privateData) {
    ...
    const createdRules = initialRules.map(([ styleClass, rule ]) => {
      return styleClass.create([], { rule, styleSheet: this });
    });
  ...
  insertRule(rule, index) {
    ...
    // If index is greater than length, then throw an IndexSizeError
    // exception (which is deprecated, so use RangeError instead).
    if (index > length) {
    ...
    try {
      ...
      newRule = csstree.parse(rule, { context: "rule" });
    }
    catch (err) {
    ...
    idlUtils.implForWrapper(this._cssRules)._list.splice(index, 0, CSSStyleRule.create([], { rule: newRule, styleSheet: this }));
