...
const serializeAtrule = {
  ...
  namespace(rule) {
    const prefix = rule.prelude.children.head.data.name;
    const namespaceURI = rule.prelude.children.tail.data.value.value.replace(/["']/g, "");

    // The literal string "@namespace", followed by a single SPACE (U+0020)
    let str = "@namespace ";

    // The serialization as an identifier of the prefix attribute (if any)
    // followed by a single SPACE (U+0020) if there is a prefix
    str += prefix ? serializeAnIdentifier(prefix) + " " : "";

    // Followed by the serialization as URL of the namespaceURI attribute
    str += serializeAURL(namespaceURI);
